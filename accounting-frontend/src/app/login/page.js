...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || t("invalidLogin"));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 bg-[#e8ecf5] relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none"
        style={{
          backgroundImage: "linear-gradient(to right,#c5cde0 1px,transparent 1px),linear-gradient(to bottom,#c5cde0 1px,transparent 1px)",
      ...
      <div className="relative z-10 w-full max-w-105 bg-white rounded-2xl shadow-md px-10 py-10">
        ...
        <div className="flex justify-center mb-7">
          <div className="w-17 h-17 rounded-xl bg-[#1b2b6b] flex flex-col items-center justify-center gap-0.5">
            <span className="text-white text-3xl font-extrabold leading-none">F</span>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              <span className={`absolute ${isRtl ? "right-3.5" : "left-3.5"} top-1/2 -translate-y-1/2 text-gray-400`}>
              ...
              <input
                ...
                className={`w-full ${isRtl ? "pr-10 pl-4 text-right" : "pl-10 pr-4"} py-3 rounded-lg border border-gray-200 bg-gray-50 text-sm text-gray-800 placeholder-gray-400 outline-none focus:ring-2 focus:ring-[#1b2b6b]/25 focus:border-[#1b2b6b] transition`}
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full ${isRtl ? "pr-10 pl-11 text-right" : "pl-10 pr-11"} py-3 rounded-lg border border-gray-200 bg-gray-50 text-sm text-gray-800 placeholder-gray-400 outline-none focus:ring-2 focus:ring-[#1b2b6b]/25 focus:border-[#1b2b6b] transition`}
              />
              <button
                type="button"
                onClick={() => setShowPass(!showPass)}
                className={`absolute ${isRtl ? "left-3.5" : "right-3.5"} top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600`}
              >
                {showPass ? (
          ...
          <div className={`flex items-center justify-between ${isRtl ? "flex-row-reverse" : ""}`}>
            ...
            <Link href="/forgot-password" className="text-sm font-semibold text-[#111827] hover:underline">
              {t("forgotPassword")}
            </Link>
          </div>
          ...
          >
            {loading ? (
              ...
              <>
                {t("signIn")}
                <svg className={`w-5 h-5 ${isRtl ? "rotate-180" : ""}`} fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24">
        ...
        <div className="space-y-3">
          <button className="w-full flex items-center justify-center gap-3 py-3 rounded-xl border border-gray-200 bg-white hover:bg-gray-50 transition text-sm font-medium text-gray-700">
